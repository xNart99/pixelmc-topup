		...
		*/
		...
		curl_close($ch);
			...
			if($out->status === '00'){
				...
				echo '<script type="text/javascript">swal("Thẻ hợp lệ", "'.$out->msg.'", "success");</script>';
				echo '<script type="text/javascript">toastr.info("Nếu nạp thành công, Ruby sẽ tự động cộng vào tài khoản của bạn.","Đang Xử Lý");;</script>';
			}else if($out->status !== '00'){
				$conn->query("INSERT INTO trans_log (name,trans_id,amount,pin,seri,type) VALUES ('".$username."','".$content."',".$menhgia.",'".$pin."','".$seri."','".$loaithe."')");
				echo '<script type="text/javascript">swal("Không hợp lệ", "'.$out->msg.'", "error");</script>';
			}
			else{
				echo '<script type="text/javascript">swal("Lỗi","Vui lòng thông báo cho BQT","error");</script>';
			}	
